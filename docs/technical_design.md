...
Channel = {
  ...
  "rotation": {                        # non-null only when space starts with ee_rotation / camera_rotation
      "repr": "axis_angle" | "rotvec" | "euler_rpy" | "euler_xyz" | "euler_zyx"
              | "quat_wxyz" | "unknown",
              # euler_rpy = the axes are named upstream but the order is not (C; ADR 009).
              # Neither euler_xyz (asserts an order nobody stated) nor unknown (discards the
              # naming that was stated) is honest for that case.
...
Typical values: A = `synthesized@10Hz` — **measured in M1, not assumed**: pusht's `timestamp` column is bit-for-bit `float32(frame_index / fps)` for all 25,650 rows, so the dataset records no acquisition time at all (ADR 005). B = `synthesized@50Hz`, **confirmed by the same measurement in M3** over all 7,500 cached rows, not inherited from A (ADR 008). C = `synthesized@5Hz` (RLDS steps have no timestamps at all, and the rate is declared in `config/sources.yaml`; the adapter refuses to default it — ADR 009); D = `annotation_seconds` + `derived_from_seconds@<extraction_fps>` (ADR 004).
...
**i. Two episode-level facts the QC ruleset needs, and neither is a frame.** M5 added the last two fields of `EpisodeMeta`, both `None` where the fact does not exist:

```python
segment: EpisodeSegment | None      # the annotation interval the episode was cut from (D)
termination_column: str | None      # the raw.* column carrying the upstream end marker (A, B)
```

- `SEGMENT_BOUNDS` judges the interval itself — start/end against the parent recording's duration, and overlap against the neighbouring intervals — so it needs `(parent_id, start_s, end_s, parent_duration_s, prev_end_s, next_start_s)`, none of which is derivable from the frames it produced. `EpisodeSegment` deliberately validates almost nothing: a value object that rejected `start_s >= end_s` would turn a reportable QC defect into an ingestion crash. Constrained by invariant 18.
- `TERMINATION_CONSISTENCY` must be told **which** raw column is the end marker, because the three sources that have one disagree completely: `next.done` for A and B, `is_terminal` for C. Guessing by name is exactly what "never trust upstream field names" forbids, so the adapter names the column and invariant 19 requires it to exist in `raw_frame_columns`.

Both are stored in the catalog (`episodes.segment_json`, `episodes.termination_column`, schema version 3), because the QC stage after a `ruleset_version` bump reads the episode back from the catalog rather than from the adapter ([ADR 015](adr/015-qc-episode-facts-and-persisted-streams.md)).
...
| `ACTION_JERK`             | Inter-frame discontinuity (jumps, packet-loss steps)                    | **physical channels only**: single-channel `abs(delta_a)` exceeds 8× that channel's **p99** within the episode, and the jump is isolated against the median step of its 5-step neighbourhood (excluding normal acceleration). The originally specified p99.9 is degenerate at these episode lengths — measured `max/p99.9` never exceeds 1.57, so a 5× rule could never fire ([ADR 014](adr/014-qc-thresholds-from-measured-distributions.md)) | REVIEW                                    | has_action                                                                           |
| `STATIC_EPISODE`          | Nearly motionless throughout / too short                                | frames < 20; or 95% of steps move no physical channel at all (bit-for-bit); or total travel below a fraction of the channel's **own declared range**, measured over **positional** channels only — a gripper's range is actuation and a quaternion component's `[-1,1]` is not a distance ([ADR 014](adr/014-qc-thresholds-from-measured-distributions.md))                                                                                    | FAIL                                      | — (the only ungated rule; it applies invariant 13 itself)                            |
...
- **Channels with `origin != "measured"` have their severity automatically downgraded one level — the fifth trap (also forced by D)**: D's camera poses are COLMAP output, so a jump in them is most likely **reconstruction failure**, not corruption; judging them as `measured` and returning FAIL amounts to using model error to discredit data. The downgrade is **applied uniformly by the domain layer** (§8.4 invariant 13); rule implementations cannot bypass it, and the `reason` must state the basis, or the report will contain a batch of unexplainable REVIEWs. A/B/C states are entirely `measured`, so this rule is an identity transform on them — **only D can verify it works**. The engine keys the downgrade off the rule's `required_levels`, so the one **ungated** rule — `STATIC_EPISODE`, which must run on any episode — applies invariant 13 itself over the channels it actually read; its length complaint is exempt, because "this episode has 4 frames" is a fact about the episode and not about the reconstruction ([ADR 014](adr/014-qc-thresholds-from-measured-distributions.md)).
- **`TERMINATION_CONSISTENCY` catches errors the other nine cannot see**: two episodes wrongly concatenated during normalization (an end signal appears mid-episode), or one episode split in half (no end signal on the final frame). **M5 corrected this note's original claim that B and D have no explicit end signal**: B does — LeRobot's v3.0 export of `aloha_sim_insertion` carries `next.done`, and what B actually lost is `terminated` vs `truncated`, a different loss (ADR 002). D has none, so `has_termination_signal=False` → a clean `SKIPPED`. C's marker exists upstream but sits on the trailing boundary steps that normalization trims, so the capability is measured on the rows we keep and is false on all 12 episodes — declaring it from the upstream record would report every C episode as truncated for a defect of our own making ([ADR 015](adr/015-qc-episode-facts-and-persisted-streams.md)).
- **Thresholds are chosen from `rdp stats`, which reads `qc_results.metrics_json` back with SQL alone** — no rule is re-run to produce the evidence, so the numbers behind a threshold are the numbers the verdicts were actually made from. Distributions are grouped per source because a jerk ratio is comparable across embodiments and a travel fraction in pixels is not. Every threshold in `config/qc.yaml` carries the measurement that justifies it, and the corpus-wide outcome (196 PASS, 6 REVIEW, 0 FAIL over 202 episodes, with each REVIEW inspected) is recorded in [ADR 014](adr/014-qc-thresholds-from-measured-distributions.md) rather than tuned away.
...
Version 3 (M5) adds `episodes.segment_json` and `episodes.termination_column`, the two episode
facts the QC ruleset needs, and version 4 adds `episodes.stream_specs_json`, which M4 declared on
the entity but never stored — a gap that only the re-QC path could expose, because every other
path re-derives the episode from its adapter
...
episodes(
  ...
  segment_json, termination_column,   -- the annotation interval and the end-marker column
...
18. `EpisodeMeta.segment` is non-null ⟹ `capabilities.is_segment`. Stated one-directionally on purpose: it constrains what we recorded, never what upstream owes us — a source that publishes no interval is simply silent.
19. `EpisodeMeta.termination_column` is non-null ⟹ `capabilities.has_termination_signal` **and** the named column is registered in `raw_frame_columns`. A rule may never be handed the name of a column the artifact does not contain ([ADR 015](adr/015-qc-episode-facts-and-persisted-streams.md)).
...
| 6   | `gripper`                | gripper      | `gripper`              | **true** | None   | normalized | true        | **ternary change command -1/0/+1**           |
...
4. **`is_first/is_last/is_terminal` and the trailing padding steps**: RLDS's final steps often carry a zero or placeholder action, and counting them into statistics pollutes `ACTION_JERK` and static detection → during normalization, trim them and record the count in `raw_extra`. M0 found something odder still in episode 0: `is_last` and `is_terminal` are set on the **final two** steps, not just the last. M3 then found the reason the obvious rule is wrong: zero-valued actions also occur **mid-episode** (episode 0 at step 68, episode 1 at step 0), where `is_last` is not set. The shipped rule therefore trims the trailing run of steps where `is_last` is truthy **and** `world_vector` and `rotation_delta` are both exactly zero — never either signal alone — and records `n_steps_upstream`, `n_trailing_boundary_steps_trimmed`, the trimmed rewards and the terminal reward in `raw_extra`, plus a `trim_trailing_steps` entry in `provenance.transforms` ([ADR 009](adr/009-rlds-identity-clock-and-padding.md), decision 4).
...
10. **It is the only source with no stable upstream episode ID**: one shard holds many episodes (train = 896 episodes over 412 shards, ~2–3 each), and a record's only handle is its position inside a shard file whose name encodes the current shard count. Identity must therefore be **layout-independent**: `upstream_id = f"{split}#{global_index:06d}"`, the episode's cumulative index within the split, derived from `dataset_info.json`'s `shardLengths`. The shard name and the offset inside it are carried in `raw_extra` as locators, never as identity. (The earlier sketch `f"{split}/{shard}#{i}"` was rejected in M3: it bakes the layout into the identity, so the moment upstream re-shards the entire old corpus is misidentified as new — landing directly on an acceptance criterion. It also contains a `/`, which the staging-directory layout would silently turn into a directory level.) `content_hash` is computed over the **normalized episode bytes** rather than the shard file. The shard layout is pinned in `config/sources.yaml` as `shard_layout_revision: "train:412-shards@0.1.0"` and folded into `adapter_version`, so declaring a new layout marks the corpus `stale` and re-verifies every hash, through machinery §5 already had ([ADR 009](adr/009-rlds-identity-clock-and-padding.md), decisions 1–2).
